  ...
  useEffect(() => {
    const fetchCancelled = async () => {
      try {
        ...
        if (res.data.success) {
          // Filtering only 'cancelled' jobs locally
          const voidedOnly = res.data.data.filter(job => job.status === 'cancelled');
          setJobs(voidedOnly);
        }
      } catch (err) {
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-12 font-sans">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-20 gap-8">
        <div className="max-w-2xl">
          ...
          <p className="text-slate-500 font-medium mt-8 leading-relaxed max-w-lg">
            This archive contains service requests that were cancelled or terminated. Review the logs below to understand the termination reasons.
          </p>
        </div>
        ...
        <div className="bg-white border-2 border-slate-100 rounded-[45px] p-10 min-w-[320px] shadow-xl shadow-slate-100/50 flex items-center justify-between relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <h2 className="text-6xl font-black text-zinc-900 tracking-tighter">{jobs.length}</h2>
          </div>
      ...
      {jobs.length > 0 ? (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
          {jobs.map((job) => (
            <div key={job._id} className="group bg-white border border-slate-100 rounded-[60px] p-2 hover:border-rose-100 transition-all duration-700 hover:shadow-2xl hover:shadow-rose-50/50">
              <div className="bg-white rounded-[55px] p-8 md:p-12">
                ...
                <div className="grid grid-cols-2 gap-y-10 gap-x-6 mb-12 ml-2">
                  <div className="flex flex-col gap-2">
                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-[2px]">Previously Assigned</span>
                    <div className="flex items-center gap-2">
                       <div className="w-5 h-5 bg-slate-100 rounded-md flex items-center justify-center text-[8px] text-slate-500 font-bold">
                         {job.workerId?.username?.charAt(0) || 'U'}
                       </div>
                       <span className="text-xs font-black text-zinc-800 uppercase tracking-tighter">
                         {job.workerId?.username || 'Unassigned'}
                       </span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <span className="text-xs font-black text-zinc-800 uppercase tracking-tighter flex items-center gap-1">
                      <FiMapPin size={14} className="text-rose-400" /> {job.location}
                    </span>
                  </div>
                  <div className="flex flex-col gap-2">
                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-[2px]">Contact Hash</span>
                    <span className="text-xs font-black text-zinc-800 uppercase tracking-tighter flex items-center gap-1">
                      <FiPhone size={14} className="text-rose-400" /> {job.phone}
                    </span>
                  </div>
                  <div className="flex flex-col gap-2">
                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-[2px]">Logged on</span>
                    <span className="text-xs font-black text-zinc-800 uppercase tracking-tighter flex items-center gap-1">
                      <FiCalendar size={14} className="text-rose-400" /> {new Date(job.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                </div>
        ...
        <div className="py-40 flex flex-col items-center justify-center bg-slate-50/30 rounded-[100px] border-2 border-dashed border-slate-100">
          ...
          <p className="text-slate-400 font-medium mt-3">No cancelled orders found in the database.</p>
        </div>
      )}
